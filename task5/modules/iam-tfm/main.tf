...
resource "aws_iam_role" "this_ecsTaskExecutionRole" {
  ...
  assume_role_policy  = file("${path.module}/policies/eccTaskExecutionAssumeRolePolicy.json")
...
resource "aws_iam_role" "this_ecsInstanceRole" {
  ...
  assume_role_policy = file("${path.module}/policies/ecsInstanceRoleAssumeRolePolicy.json")
...
resource "aws_iam_role_policy" "this_ecsInstanceRolePolicy" {
  ...
  policy = file("${path.module}/policies/ecsInstancerolePolicy.json")
}
